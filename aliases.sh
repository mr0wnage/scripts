...
alias !stakes='echo "Active Stake:" && solana stakes /root/solana/vote-account-keypair.json | grep -w "Active Stake:" | awk '{print $3}' | awk '{s+=$1} END {print s}' && echo "Activating Stake:" && solana stakes /root/solana/vote-account-keypair.json | grep -i " activates" -A5 -B5 | grep Balance | awk '{print $2}' | paste -sd+ | bc && echo "Deactivating Stake:" && solana stakes /root/solana/vote-account-keypair.json | grep -i "deactivates" -A5 -B5 | grep Balance | awk '{print $2}' | paste -sd+ | bc'
alias !journal='journalctl -u solana.service -f --no-hostname | ccze'
