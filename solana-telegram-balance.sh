...
ACTIVATING_STAKE=$(${APP_SOLANA} stakes ${KEY_VOTE} | grep -i "Activating" | awk '{print $3}' | awk '{s+=$1} END {printf("%d\n", s+0.5)}')
DEACTIVATING_STAKE=$(${APP_SOLANA} stakes ${KEY_VOTE} | grep -i 'deactivates' -B5 | grep Balance | awk '{s+=$2} END {printf("%d\n", s+0.5)}')
